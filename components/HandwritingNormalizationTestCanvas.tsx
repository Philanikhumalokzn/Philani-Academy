...
export default function HandwritingNormalizationTestCanvas() {
  ...
  const debugSections = useMemo<DebugSection[]>(() => {
    ...
    const roles = analysis.roles.map((role) => `${role.groupId}: ${role.role} label=${role.qualifiedRoleLabel || `${role.role}-unknown`} symbol=${role.recognizedSymbol?.value || 'unknown'} family=${role.descriptor.family} depth=${role.depth} score=${role.score.toFixed(2)}${role.parentGroupId ? ` parent=${role.parentGroupId}` : ''}${role.containerGroupIds.length ? ` containers=${role.containerGroupIds.join(',')}` : ''}${role.evidence.length ? ` :: ${role.evidence.join(' | ')}` : ''}`)
