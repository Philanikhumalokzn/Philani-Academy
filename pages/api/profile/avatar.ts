...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!fileSize || fileSize > MAX_AVATAR_SIZE) {
      return res.status(400).json({ message: 'Avatar must be under 4 MB' })
    }

    const filename = buildFilename(upload)
    const relativePath = path.posix.join('uploads', 'avatars', userId, filename).replace(/\\/g, '/')
