...
export function PostReplyComposerSurface({
  blocks,
  draftText,
  editingTarget,
  viewerName,
    ...
    <div className="flex flex-col gap-2">
      {(submitting || imageUploading) ? (
        <div className="flex justify-end px-1">
          <span className="text-[11px] font-medium text-slate-500">{submitting ? 'Sending...' : 'Uploading image...'}</span>
        </div>
      ) : null}
            ...
            {composerVisibleBlocks.map((block, index) => {
              ...
              if (block.type === 'text') {
                return (
                  <div
                    ...
                    className="philani-gradient-outline-soft [--philani-outline-fill:#f8fafc] rounded-2xl px-3 py-2 text-sm leading-6 whitespace-pre-wrap break-words text-slate-700"
                    onClick={() => onEditBlock(block, index)}
                    onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onEditBlock(block, index) } }}
                    {...blockHandlers}
                  >
                    {block.text}
                  </div>
                ...
                return (
                  <div key={block.id} className="pt-1">
                    <div
                      ...
                      onContextMenu={(e) => { e.preventDefault(); onOpenBlockCrudOptions(blockTarget) }}
                    >
                      <PublicSolvePlainExcalidrawViewer
                        scene={block.scene}
                        viewerHeightPx={180}
              ...
              return (
                <div
                  ...
                  role="button"
                  tabIndex={0}
                  className="philani-gradient-outline-soft [--philani-outline-fill:#f8fafc] rounded-2xl px-3 py-2"
                  onClick={() => onEditBlock(block, index)}
                  onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onEditBlock(block, index) } }}
                  {...blockHandlers}
                >
