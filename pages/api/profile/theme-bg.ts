...
const MAX_THEME_BG_SIZE = 8 * 1024 * 1024 // 8 MB
...
function extensionFor(file: File) {
  ...
  return (fromMime || fromName || '.jpg').toLowerCase()
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!upload) {
      return res.status(400).json({ message: 'Background image file is required' })
    }
    ...
    if (!fileSize || fileSize > MAX_THEME_BG_SIZE) {
      return res.status(400).json({ message: 'Background image must be under 8 MB' })
    }

    const filename = buildFilename(upload)
    const relativePath = path.posix.join('uploads', 'theme-bg', userId, filename).replace(/\\/g, '/')
    ...
    await (prisma as any).user.update({
      where: { id: userId },
      data: { profileThemeBgUrl: publicUrl },
    })
    ...
    const message = error?.message?.includes('maxFileSize')
      ? 'Background image must be under 8 MB'
