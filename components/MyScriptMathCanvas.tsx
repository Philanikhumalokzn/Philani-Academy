...
const MyScriptMathCanvas = ({ gradeLabel, roomId, userId, userDisplayName, isAdmin, boardId, uiMode = 'default', defaultOrientation, overlayControlsHandleRef, onOverlayChromeVisibilityChange }: MyScriptMathCanvasProps) => {
  ...
  const showSideControls = Boolean(useStackedStudentLayout && (isOverlayMode || isCompactViewport))
  ...
  const updateHorizontalScrollbarDrag = useCallback((event: React.PointerEvent) => {
    ...
    const target = horizontalPanDragRef.current.startScrollLeft + ratioDx * maxScroll
  ...
  return (
    <div className={isOverlayMode ? 'h-full' : undefined}>
      <div className={`flex flex-col gap-3${isOverlayMode ? ' h-full min-h-0' : ''}`}>
        ...
        {/* Side controls (compact stacked): left = save/undo/redo, right = send + gear. */}
        {showSideControls && (
          <>
            {canPersistLatex && (
              <div className="fixed left-2 top-1/2 -translate-y-1/2 z-[500] flex flex-col gap-2">
                <button
                  type="button"
            ...
            {isAdmin && (
              <div className="fixed right-2 top-1/2 -translate-y-1/2 z-[500] flex flex-col gap-2">
                <button
                  type="button"
                  className="p-2 text-slate-700 disabled:opacity-50"
                  title="Send step"
