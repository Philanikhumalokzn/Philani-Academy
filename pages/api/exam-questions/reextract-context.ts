...
function buildContextExtractionPrompt(
  ...
  return [
    ...
    `- topic: choose EXACTLY ONE label from this fixed list and return it EXACTLY as written (strict parsing requirement): ${topicList}`,
    `- topic strictness rules: do not invent, paraphrase, merge, or partially rewrite topic names; if unsure, return "Other"`,
    `- root-topic consistency rule: because this preamble is shared by all sub-questions under QUESTION ${rootNumber}, all of them must use the same topic label`,
