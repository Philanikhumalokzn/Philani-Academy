...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!fileSize || fileSize > MAX_COVER_SIZE) {
      return res.status(400).json({ message: 'Cover image must be under 8 MB' })
    }

    const filename = buildFilename(upload)
    const relativePath = path.posix.join('uploads', 'covers', userId, filename).replace(/\\/g, '/')
    ...
    await (prisma as any).user.update({
      where: { id: userId },
      data: { profileCoverUrl: publicUrl },
    })
    ...
    const message = error?.message?.includes('maxFileSize')
      ? 'Cover image must be under 8 MB'
