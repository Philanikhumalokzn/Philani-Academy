const url = 'https://philani-academy.vercel.app/api/signup';
...
async function run() {
  ...
  try {
    ...
    const p = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
    console.log('Status:', p.status)
    const pt = await p.text()
    try {
      console.log('JSON:', JSON.parse(pt))
    } catch (e) {
      console.log('Body:', pt)
    }
  } catch (e) {
