...
function rootOfQuestionNumber(value) {
  const q = normalizeQuestionNumber(value)
  if (!q) return ''
  return q.split('.')[0] || q
}
...
async function run() {
  ...
  try {
    const sources = await prisma.resourceBankItem.findMany({
      where: { grade, parsedJson: { not: null } },
    ...
    for (const source of sources) {
      ...
      const unique = Array.from(new Set(questionNumbers)).sort((a, b) => compareQuestionNumbers(a, b))
      ...
      const anns = await prisma.questionAnnotation.findMany({
        where: { sourceId: source.id, questionNumber: { in: unique } },
        select: { questionNumber: true, topic: true },
      })
      const byQ = new Map(anns.map((row) => [normalizeQuestionNumber(row.questionNumber), String(row.topic || '').trim()]))

      const rootTopicSets = new Map()
      for (const q of unique) {
        const topic = byQ.get(normalizeQuestionNumber(q)) || ''
        if (!topic) missingTopic += 1
        if (topic === 'Calculus') calculusTopic += 1

        const root = rootOfQuestionNumber(q)
        if (!rootTopicSets.has(root)) rootTopicSets.set(root, new Set())
        if (topic) rootTopicSets.get(root).add(topic)
      }

      for (const set of rootTopicSets.values()) {
        if (set.size > 1) inconsistentRoots += 1
      }
    }

    console.log('GRADE_TOPIC_VERIFY:' + JSON.stringify({
      grade,
      papers,
      totalQuestions,
      missingTopic,
