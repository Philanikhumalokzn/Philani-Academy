...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (jaasPriv && jaasKid && jaasApp) {
    try {
      ...
      const features = {
        ...
        recording: true,
        flip: false,
        'lobby-mode': isModerator ? 'disabled' : 'enabled'
      }
  ...
  const payload = { aud: appId, iss: apiKey || appId, sub: apiKey, room: roomSegment, exp, context: { features: { 'lobby-mode': isModerator ? 'disabled' : 'enabled' } } }
